import type { template as templateEffectShape } from '@fiction/cards/effectShape'
...
async function getEffects(args: { factory: CardFactory }) {
  const { factory } = args
  const shapeEffect = await factory.fromTemplate<typeof templateEffectShape>({
    templateId: 'effectShape',
    userConfig: {
      shapes: [
        {
          shape: 'circle',
          position: { origin: 'topLeft', offsetX: 20, offsetY: 5 },
          style: { scale: 1.5, opacity: 10 },
        },
      ],
    },
  })
...
export async function getConfig(args: { templateId: string, factory: CardFactory }): Promise<ConfigResponse> {
  ...
  const shapeEffect = await getEffects({ factory })
  return {
    schema,
    options,
    ...
    demoPage: {
      cards: [
        { templateId: 'statement', userConfig: await getUserConfig(), effects: [shapeEffect] },
        { templateId: 'statement', userConfig: await getDemoConfig({ stock }), effects: [shapeEffect] },
      ],
    },
  }
}
